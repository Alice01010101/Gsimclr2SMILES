...
class G2SDataset(Dataset):
    def __init__(self, args,is_reac,file: str):
        ...
        self.data_size = len(self.src_token_ids) #数据总长度
...
def get_graph_features_from_smi(i, smi, use_rxn_class):
    ...
    #########################################################
    #将它们的数据类型转换为tensor，之后可调用torch.cat函数进行拼接
    """
    b_features=torch.from_numpy(b_features)
    a_graphs=torch.from_numpy(a_graphs)
    b_graphs=torch.from_numpy(b_graphs)
    """
    #########################################################
