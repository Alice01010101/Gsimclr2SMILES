...
from my_utils.data_utils import get_graph_features_from_smi, load_vocab, make_vocab, \
    tokenize_selfies_from_smiles, tokenize_smiles
from my_utils.train_utils import log_tensor, set_seed, setup_logger
...
def binarize_g2s(src_file: str, tgt_file: str, prefix: str, output_path: str,
    ...
    graph_features_and_lengths = p.imap(
        ...
        ((i, "".join(line.split()), False) for i, line in enumerate(src_lines)) #对图处理提取特征，只处理了src_lines
    )
         
    p.close()
    p.join()
    """
    src_graph_features_and_lengths=[]
    for i,line in enumerate(src_lines):
        src_graph_features_and_lengths.append(
            get_graph_features_from_smi(i, "".join(line.split()), False)
        )
    #src_graph_features_and_lengths = list(src_graph_features_and_lengths)

    tgt_graph_features_and_lengths=[]
    for i,line in enumerate(tgt_lines):
        tgt_graph_features_and_lengths.append(
            get_graph_features_from_smi(i, "".join(line.split()), False)
        )
    #tgt_graph_features_and_lengths = list(tgt_graph_features_and_lengths)
    
    logging.info(f"Done graph featurization, time: {time.time() - start}. Collating and saving...")

    src_a_scopes, src_a_scopes_lens, src_b_scopes, src_b_scopes_lens, src_a_features, src_a_features_lens, \
        src_b_features, src_b_features_lens, src_a_graphs, src_b_graphs = zip(*src_graph_features_and_lengths)

    src_a_scopes = np.concatenate(src_a_scopes, axis=0)
    src_b_scopes = np.concatenate(src_b_scopes, axis=0)
    src_a_features = np.concatenate(src_a_features, axis=0)
    src_b_features = np.concatenate(src_b_features, axis=0)
    src_a_graphs = np.concatenate(src_a_graphs, axis=0)
    src_b_graphs = np.concatenate(src_b_graphs, axis=0)

    src_a_scopes_lens = np.array(src_a_scopes_lens, dtype=np.int32)
    src_b_scopes_lens = np.array(src_b_scopes_lens, dtype=np.int32)
    src_a_features_lens = np.array(src_a_features_lens, dtype=np.int32)
    src_b_features_lens = np.array(src_b_features_lens, dtype=np.int32)

    tgt_a_scopes, tgt_a_scopes_lens, tgt_b_scopes, tgt_b_scopes_lens, tgt_a_features, tgt_a_features_lens, \
        tgt_b_features, tgt_b_features_lens, tgt_a_graphs, tgt_b_graphs = zip(*tgt_graph_features_and_lengths)

    tgt_a_scopes = np.concatenate(tgt_a_scopes, axis=0)
    tgt_b_scopes = np.concatenate(tgt_b_scopes, axis=0)
    tgt_a_features = np.concatenate(tgt_a_features, axis=0)

    #############################################################
    tgt_b_features = np.concatenate(tgt_b_features, axis=0)
    tgt_a_graphs = np.concatenate(tgt_a_graphs, axis=0)
    tgt_b_graphs = np.concatenate(tgt_b_graphs, axis=0)
    """
    tgt_b_features = torch.cat(tgt_b_features, 0)
    tgt_a_graphs = torch.cat(tgt_a_graphs, 0)
    tgt_b_graphs = torch.cat(tgt_b_graphs, 0)

    tgt_b_features=np.array(tgt_b_features,dtype=np.int32)
    tgt_a_graphs=np.array(tgt_a_graphs,dtype=np.int32)
    tgt_b_graphs=np.array(tgt_b_graphs,dtype=np.int32)
    """
    #############################################################

    tgt_a_scopes_lens = np.array(tgt_a_scopes_lens, dtype=np.int32)
    tgt_b_scopes_lens = np.array(tgt_b_scopes_lens, dtype=np.int32)
    tgt_a_features_lens = np.array(tgt_a_features_lens, dtype=np.int32)
    tgt_b_features_lens = np.array(tgt_b_features_lens, dtype=np.int32)
    np.savez(
        output_file,
        src_token_ids=src_token_ids,
        src_lens=src_lens,
        tgt_token_ids=tgt_token_ids,
        tgt_lens=tgt_lens,
        src_a_scopes=src_a_scopes,
        src_b_scopes=src_b_scopes,
        src_a_features=src_a_features,
        src_b_features=src_b_features,
        src_a_graphs=src_a_graphs,
        src_b_graphs=src_b_graphs,
        src_a_scopes_lens=src_a_scopes_lens,
        src_b_scopes_lens=src_b_scopes_lens,
        src_a_features_lens=src_a_features_lens,
        src_b_features_lens=src_b_features_lens,

        tgt_a_scopes=tgt_a_scopes,
        tgt_b_scopes=tgt_b_scopes,
        tgt_a_features=tgt_a_features,
        tgt_b_features=tgt_b_features,
        tgt_a_graphs=tgt_a_graphs,
        tgt_b_graphs=tgt_b_graphs,
        tgt_a_scopes_lens=tgt_a_scopes_lens,
        tgt_b_scopes_lens=tgt_b_scopes_lens,
        tgt_a_features_lens=tgt_a_features_lens,
        tgt_b_features_lens=tgt_b_features_lens
    )
