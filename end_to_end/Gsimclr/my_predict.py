...
import sys
import torch
from my_models.graph2seq_series_rel import Graph2SeqSeriesRel
from torch.utils.data import DataLoader
from my_utils import parsing
from my_utils.data_utils import canonicalize_smiles, load_vocab, G2SDataset
from my_utils.train_utils import log_tensor, param_count, set_seed, setup_logger
...
def main(args):
    ...
    if args.do_predict and os.path.exists(args.result_file):
        ...
        #####################################################
        saved_model = torch.load(args.load_from)
        pretrain_args = saved_model["args"]
        #####################################################
        ...
        state_dict={k:v for k,v in saved_model['state_dict'].items() if k in model_dict.keys()}
        model_dict.update(state_dict)
        pretrain_state_dict = model_dict
        ########################################################################################
        ...
        # initialization ----------------- data
        test_dataset = dataset_class(pretrain_args, is_reac=True, file=args.test_bin)
        test_dataset.batch(
        ...
        with torch.no_grad():
            for test_idx, test_batch in enumerate(test_loader):
                ...
                #validation是token_ids进行比较，test是将token_ids再进一步提取成tokens再比较
