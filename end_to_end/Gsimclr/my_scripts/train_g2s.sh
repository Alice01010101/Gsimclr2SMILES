...
ACCUM_COUNT=4
ENC_PE=none
ENC_H=256 #256 为D-MPNN的维度，与下面encoder的维度要对齐
BATCH_SIZE=8 #4096
ENC_EMB_SCALE=sqrt
...
CUDA_VISIBLE_DEVICES="2,3" python -m torch.distributed.launch --nproc_per_node 2 end_to_end-train.py \
  --model="$MODEL" \
  --data_name="$DATASET" \
  --task="$TASK" \
  --representation_end=$REPR_END \
  --load_from="$LOAD_FROM" \
  --train_bin="./my_preprocessed/$PREFIX/val_0.npz" \
